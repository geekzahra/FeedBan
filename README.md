...
FeedBan یک ابزار کوچک برای X (توییتر) است. شما یک ایموجی یا عبارت به آن می‌دهید و FeedBan همان مورد را داخل نام نمایشی یا بیوی حساب‌ها پیدا می‌کند.

FeedBan دو حالت دارد:
...
### FeedBan دقیقاً چه چیزهایی را بررسی می‌کند؟

FeedBan این موارد را بررسی می‌کند:

- نام نمایشی حساب
- بیویی که در همان لحظه داخل صفحه یا کارت کاربر دیده می‌شود
- متن معمولی، ایموجی، پرچم و ایموجی‌های ترکیبی

FeedBan متن داخل پست‌ها را بررسی **نمی‌کند**. مثلاً اگر کسی 🎒 را فقط داخل یک پست نوشته باشد، ولی این ایموجی در نام یا بیوی او نباشد، آن حساب پیدا نمی‌شود.
...
حالا باید پنل **FeedBan 🛡️** را پایین سمت راست صفحه ببینید.
...
FeedBan در اولین اجرا با حالت آزمایشی روشن می‌شود؛ پس در این تست نباید کسی بلاک شود.
...
FeedBan تلاش می‌کند منوی X را باز کند، گزینه Block را بزند و آن را تأیید کند. تعداد بلاک‌ها و نتیجه تلاش‌ها در شمارنده‌ها و بخش Recent activity نمایش داده می‌شود.

اگر در زمان کوتاه تعداد زیادی بلاک انجام شود، FeedBan موقتاً کار را متوقف می‌کند. این توقف ریسک را کمتر می‌کند، اما تضمین نمی‌کند که X این نوع استفاده خودکار را بپذیرد.

### توقف یا حذف FeedBan

- برای توقف بررسی صفحه، **Automatic scanning** را خاموش کنید.
- برای کوچک کردن پنل، روی دکمه `−` بزنید.
- برای غیرفعال کردن کامل، Tampermonkey را باز کنید و FeedBan را خاموش کنید.
- برای حذف، وارد داشبورد Tampermonkey شوید و FeedBan را پاک کنید.
...
- مطمئن شوید FeedBan داخل Tampermonkey روشن است.
...
- FeedBan را خاموش کنید.
- تب گیرکرده X را کامل ببندید.
- X را در یک تب جدید باز کنید.
- قبل از روشن کردن دوباره، مطمئن شوید جدیدترین نسخه FeedBan را دارید.

پیام‌های کنسول مرورگر درباره تبلیغات، Content Security Policy یا `beforeinstallprompt` معمولاً مربوط به خود X هستند و خطای FeedBan محسوب نمی‌شوند.

### حریم خصوصی

FeedBan داخل مرورگر شما اجرا می‌شود. تنظیمات، فیلترها، شمارنده‌ها و سابقه فعالیت آن توسط افزونه مدیریت اسکریپت ذخیره می‌شوند.

FeedBan توکن ورود X را جمع‌آوری نمی‌کند و فیلترهای شما را به سرور جداگانه‌ای برای FeedBan نمی‌فرستد.
